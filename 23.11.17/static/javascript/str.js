...
// //사용된다. String객체에는 여러가지 다양한 문자열을 처리하기위한 메서드가 있다.
...
// // 7.문자열추출 - substring()
// console.log(word.substring(0,7)); //8번쨰 부터 7번 이전까지 추출
...
// // 강동욱이 보유한 현금에 서종우의 돈을 뺴면 얼마인가??
// // 송재영의 돈을 434,329원으로 변경하시오

// // var nam  = text.split("-")[1];
// // var nam2 = nam.replace(/\D/g,"");
// // console.log(nam2);

// // var kang =  text.split("-")[0].replace(/\D/g,"");
// // var seo = text.split("-")[2].replace(/\D/g,"");
// // console.log(kang-seo);

// // console.log(text.replace("4,343,290","434,329"));
...
// console.log(word.padStart(10)); // 앞에 5 빈공간
// console.log(word.padEnd(10,"pr"));
...
// .indexOf(문자열) - 문자열에서 특정문자 또는 문자열이 몇번쨰 있는지 알려준다. 없으묜 -1
// .lastIndexOf(문자열) - 문자열의 끝에서 문자또는 문자열 몇번쨰있는지 검색
...
let data = info.split(" - ");
//출력 - 이름 직업
var 이순신_name = data[0].substring(0,data[0].indexOf(":"));
var 이순신_직업 = data[0].substring(data[0].indexOf(":")+1,data[0].indexOf(","));
console.log(이순신_name +" "+ 이순신_직업);

//장보고의 나이는 몇인가? 츌력 - 나이만 출력하기!
var 장보고_나이 = data[1].substring(data[1].indexOf(",")+1,data[1].indexOf(",")+3);
console.log(장보고_나이);
//parseInt() - 앞쪽에 있는 숫자만 정수로 변환
