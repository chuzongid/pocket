...
//option태그에 사용할 택스트를 위해 importStyle객체를 먼저 생성한다.
  ...
  //$.each(배열,function(index,value){}) i:배열의 인덱스 ,v:배열의값
...
function save(){  //등록버튼 클릭하면 동작함수
  ...
  //memo_list배열에 memo객체를 저장 후 저장된 memo객ㅇ체의 인덱스 구하기
...
function del_memo(n){
  ...
  for(var i in memo_list){
    if(n==memo_list[i].num){
      memo_list.splice(i,1);
    }
  }

  $(".memoList.empty();")
  for(var v of memo_list){
    $(".memoList").prepend(v.create(v.num));
  }
/*
  맵 만들어오기
  가로 7칸 세로 7칸의 정사각형
  대한민국과 일본 도시 이름으로 칸을 채워라
...
// var select = document.getElementById("import");
  ...
  // 태그의속성 추가 - 태그.setAttribute("id","myname"); ->태그에 id를 넣어준다, 가져오는건 getAttribute

  //createElement("div") ->$("div")
  //createTextNode("아하") ->.text("아하")
  //appendChild(태그또는텍스트) ->.appned(태그 또는 텍스트)
