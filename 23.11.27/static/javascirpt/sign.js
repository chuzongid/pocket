$(function(){
  ...
  $(".ability_list").on("click",function(){
    ...
    if(chk_count==all_check){ //체크된 갯수와 총 갯수를 비교
      $("#all").prop("checked",true); //체크갯수와 총 갯수가 같다면 전체 체크
    }else{
      $("#all").prop("checked",false);  //체크갯수와 총 갯수가 다르면 전체 체크박스에 체크해제
    }
  });
  ...
  $("#join").on("click",function(){
    ...
    let chk=''; //체크박스에 체그된 값들 저장 변수
    $.each($(".ability_list:checked"),function(){ //체크되어있는 태그만 반복문처리
      chk +=$(this).val()+" ";
    });
    alert(chk);
