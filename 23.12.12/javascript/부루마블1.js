...
// 전역변수 
let fund = 0; //사회복지기금 모든 금액 저장 변수
let island_ = new Array(); //무인도에 도착한 플레이어
let zone =""; //각 구역의 객체 저장 배열
...
function game_init(){
  ...
  var pcolor = ["#ff0000","#00ff00","#0000ff","#FFBB00","#0054FF"]
  for(var i=1; i<=pc; i++){
  ...
  // var idx = find_location(30);
  // console.log(idx);

}
function create_dice(){
  var dice = `<div id = 'dice_wrap'>
                <div class='dice'>
                  <div class='diceImg'>
                    <img id='dice1' src='./image/dice1.png'>
                  </div>
                  <div class='diceImg'>
                    <img id='dice2' src='./image/dice4.png'>
                  </div>
                </div>
                <div class='dicebt'>
...
function find_location( n ){ //플레이어 말이 표시될 위치 또는 이동할 위치찾기
  var index = 0;
  $(".zone").each( function( idx , item ){
    ...
    if( num == n ){
      ...
      //each 안에 모든 익명함수만 종료시키기 떄문에 find_location함수를
      //통해 return(반환) 하는게 불가능하다 . 그래서 값을 index 변수에
      //저장하여 index 변수를 반환 시켜준것이다
    }
  });
  return index;
}
...
$(function(){
  ...
  //zone배열의 내용은 어제와 동일하다
  //zone_draw() 함수는 zone배열의 값을 가지고와서 화면에 표시한다.
  //하지만 zone_draw()함수가 정상적으로 동작하지 못하고있다.
  //getJSON의 function(data){} 안으로 zone_draw 함수를 넣어주면 정상적으로 작동
  //비동기문제 발생
  //비동기처리 코드가 실행이 완료되면 그다음 실행 될 수 있게하는방법
  //async , await
